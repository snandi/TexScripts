...
\item[(d)] When fitting $x_2$ with treatment, controlling for baseline does not have any effect on the estimate or standard error of $\beta_z$. In addition to the three approaches suggested a fourth model was fit, where $x_1$ was included as a covariate. However, it did not seem to improve the fit. From fig (\ref{fig:Fig1a}) it is clear that $x_2$ is not as correlated with $x_0$ as $x_1$ is. So, the model fit result is not surprising. \\
